...
\begin{tcolorbox}[enhanced jigsaw, breakable, toprule=.15mm, opacitybacktitle=0.6, bottomrule=.15mm, colframe=quarto-callout-tip-color-frame, bottomtitle=1mm, left=2mm, leftrule=.75mm, opacityback=0, colback=white, coltitle=black, arc=.35mm, toptitle=1mm, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Solución}, titlerule=0mm, rightrule=.15mm, colbacktitle=quarto-callout-tip-color!10!white]
...
\begin{exercise}[]\protect\hypertarget{exr-chi-cuadrado-genotipo-respuesta-dieta}{}\label{exr-chi-cuadrado-genotipo-respuesta-dieta}

En un ensayo se estudia si un polimorfismo en el gen FTO influye en la
respuesta a una dieta hipocalórica. El genotipo de los participantes se
determina mediante un SNP de tipo A/T y se clasifica a los participantes
en tres grupos según su genotipo: homocigotos AA, heterocigotos AT y
homocigotos TT. Después de 6 meses de dieta, se clasifica a los
participantes en dos grupos según su respuesta a la dieta:
\emph{respondedores} (pérdida de peso clínicamente relevante ≥5\%) y
\emph{no respondedores}. Los datos obtenidos están recogidos en el
fichero
\href{datos/genotipo-respuesta-dieta.csv}{genotipo-respuesta-dieta.csv}
y se corresponden con la siguiente tabla de contingencia:
...
AT & 40 & 50 & 90 \\
TT & 25 & 35 & 60 \\
\textbf{Total} & 125 & 125 & 250 \\
\end{longtable}

Contrastar si la respuesta a la dieta depende del genotipo FTO con un
...
\FunctionTok{library}\NormalTok{(knitr)}
\FunctionTok{library}\NormalTok{(broom)}
\CommentTok{\# Cargamos los datos en un data frame}
\NormalTok{df }\OtherTok{\textless{}{-}} \FunctionTok{read.csv}\NormalTok{(}\StringTok{"https://aprendeconalf.es/master{-}nutrigenomica{-}ejercicios/datos/genotipo{-}respuesta{-}dieta.csv"}\NormalTok{)}
\CommentTok{\# Creamos la tabla de contingencia}
\NormalTok{tabla\_contingencia }\OtherTok{\textless{}{-}} \FunctionTok{table}\NormalTok{(df}\SpecialCharTok{$}\NormalTok{Genotipo, df}\SpecialCharTok{$}\NormalTok{Respuesta)}
\FunctionTok{kable}\NormalTok{(tabla\_contingencia)}
\end{Highlighting}
\end{Shaded}
...
Si no disponemos del fichero de datos, podemos crear directamente la
tabla de contingencia con los datos proporcionados.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Creamos la tabla de contingencia}
\NormalTok{tabla\_contingencia }\OtherTok{\textless{}{-}} \FunctionTok{matrix}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\DecValTok{40}\NormalTok{, }\DecValTok{60}\NormalTok{, }\DecValTok{50}\NormalTok{, }\DecValTok{40}\NormalTok{, }\DecValTok{35}\NormalTok{, }\DecValTok{25}\NormalTok{), }\AttributeTok{nrow =} \DecValTok{3}\NormalTok{, }\AttributeTok{byrow =} \ConstantTok{TRUE}\NormalTok{)}
\FunctionTok{dimnames}\NormalTok{(tabla\_contingencia) }\OtherTok{\textless{}{-}} \FunctionTok{list}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\StringTok{"AA"}\NormalTok{, }\StringTok{"AT"}\NormalTok{, }\StringTok{"TT"}\NormalTok{), }\FunctionTok{c}\NormalTok{(}\StringTok{"Respondedor"}\NormalTok{, }\StringTok{"No respondedor"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{tidy}\NormalTok{(}\FunctionTok{chisq.test}\NormalTok{(tabla\_contingencia)) }\SpecialCharTok{|\textgreater{}} 
    \FunctionTok{kable}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
